...
def view_files_role(abs_path, req_path, route, role_name):
        if os.path.isfile(abs_path):
             with open(abs_path, 'rb') as f:
	         content=f.read()
             return render_template('view_multi_files2.html', content=content, file_path=abs_path)	     
        final_files = []
        if os.path.isdir(abs_path):
            files = os.listdir(abs_path)
            for files in files:
...
def editVar(req_path, test):
	...
	print contents
        f.close()
        return render_template('view_multi_files2.html', content=contents, file_path=var_path)
...
def Ansible_test_Job():
    ...
    contents =f.read()
    f.close()
    return render_template('testbed_ansible_yaml2.html',data=contents)
